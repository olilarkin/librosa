...
```bash
# From the librosa_cpp/tests/crossval directory
cd /path/to/librosa_cpp/tests/crossval

# Make sure Python librosa is installed
pip install librosa
...
```bash
# From the librosa_cpp build directory
cd /path/to/librosa_cpp/build

# Configure with cross-validation tests enabled
cmake .. -DLIBROSA_BUILD_CROSSVAL_TESTS=ON
