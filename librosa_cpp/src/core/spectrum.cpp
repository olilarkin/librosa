...
ArrayXXc stft(const ArrayXr& y, int n_fft,
    ...
    // FFTW setup
    std::vector<double> fft_input(n_fft);
    fftw_complex* fft_output = fftw_alloc_complex(n_freq);
    fftw_plan plan = fftw_plan_dft_r2c_1d(n_fft, fft_input.data(), fft_output, FFTW_ESTIMATE);
    ...
    for (Eigen::Index t = 0; t < n_frames; ++t) {
        ...
        for (int f = 0; f < n_freq; ++f) {
            stft_matrix(f, t) = Complex(fft_output[f][0], fft_output[f][1]);
        }
    }
...
ArrayXr istft(const ArrayXXc& stft_matrix,
    ...
    // FFTW setup
    fftw_complex* ifft_input = fftw_alloc_complex(n_freq);
    std::vector<double> ifft_output(n_fft);
    fftw_plan plan = fftw_plan_dft_c2r_1d(n_fft, ifft_input, ifft_output.data(), FFTW_ESTIMATE);

    // Process each frame
    for (Eigen::Index t = 0; t < n_frames; ++t) {
        // Copy to input
        for (int f = 0; f < n_freq; ++f) {
            ifft_input[f][0] = stft_matrix(f, t).real();
            ifft_input[f][1] = stft_matrix(f, t).imag();
        }
...
ArrayXc fmt(const ArrayXr& y, Real t_min, std::optional<int> n_fmt_opt,
    ...
    std::vector<double> fft_input(n_fmt);
    for (int i = 0; i < n_fmt; ++i) {
        fft_input[i] = windowed(i);
    }

    fftw_complex* fft_output = fftw_alloc_complex(n_freq);
    fftw_plan plan = fftw_plan_dft_r2c_1d(n_fmt, fft_input.data(), fft_output, FFTW_ESTIMATE);
    fftw_execute(plan);

    ArrayXc result(n_freq);
    for (int i = 0; i < n_freq; ++i) {
        result(i) = Complex(fft_output[i][0], fft_output[i][1]);
    }
