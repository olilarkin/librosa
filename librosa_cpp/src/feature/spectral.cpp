...
ArrayXXr melspectrogram(
    ...
    MatrixXr result = mel_basis.matrix() * S.matrix();
...
ArrayXXr chroma_stft(
    ...
    MatrixXr raw_chroma = chromafb.matrix() * S.matrix();

    // Normalize
    ArrayXXr result = util::normalize(raw_chroma.array(), norm, 0);
...
ArrayXXr chroma_cqt(
    ...
    MatrixXr chroma = cq_to_chr.matrix() * C.matrix();

    // Apply threshold
    ArrayXXr result = chroma.array();
    if (threshold > 0.0) {
...
ArrayXXr chroma_cens(
    ...
    ArrayXXr cq_to_chr = filters::cq_to_chroma(
        static_cast<int>(C.rows()), bins_per_octave, n_chroma, fmin);
    MatrixXr chroma_raw = cq_to_chr.matrix() * C.matrix();
    ArrayXXr chroma = chroma_raw.array();
...
ArrayXXr chroma_vqt(
    ...
    MatrixXr chroma = vq_to_chr.matrix() * V.matrix();
...
ArrayXXr tonnetz(const ArrayXXr& chroma) {
    ...
    MatrixXr result = phi.matrix() * chroma_norm.matrix();
