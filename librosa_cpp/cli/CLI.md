...
```bash
cd librosa_cpp
cmake -B build -DLIBROSA_BUILD_CLI=ON
cmake --build build --target librosa-cpp
```

The binary is produced at `build/librosa-cpp`.

## Usage

```
librosa-cpp [OPTIONS] <file> <SUBCOMMAND> [SUBCOMMAND OPTIONS]
```
...
```bash
librosa-cpp file.wav tempo
librosa-cpp file.wav tempo --start-bpm 140
```
...
```bash
librosa-cpp file.wav beat
librosa-cpp file.wav beat --tightness 200
```
...
```bash
librosa-cpp file.wav onset
librosa-cpp file.wav onset --backtrack
```
...
```bash
librosa-cpp file.wav pitch
librosa-cpp file.wav pitch --method yin --fmin 80 --fmax 1000
```
...
```bash
librosa-cpp file.wav mfcc
librosa-cpp file.wav mfcc --n-mfcc 13 --n-mels 40
```
...
```bash
librosa-cpp file.wav melspectrogram
librosa-cpp file.wav melspectrogram --n-mels 64
```
...
```bash
librosa-cpp file.wav chroma
librosa-cpp file.wav chroma --variant cqt
```
...
```bash
librosa-cpp file.wav spectral-centroid
```
...
```bash
librosa-cpp file.wav spectral-bandwidth
```
...
```bash
librosa-cpp file.wav spectral-rolloff
librosa-cpp file.wav spectral-rolloff --roll-percent 0.95
```
...
```bash
librosa-cpp file.wav spectral-flatness
```
...
```bash
librosa-cpp file.wav spectral-contrast
librosa-cpp file.wav spectral-contrast --n-bands 4
```
...
```bash
librosa-cpp file.wav trim
librosa-cpp file.wav trim --top-db 30
```
...
```bash
librosa-cpp file.wav hpss -o harmonic.wav
librosa-cpp file.wav hpss -o percussive.wav --component percussive
```
...
```bash
# Quick file overview
librosa-cpp song.wav info

# Get tempo estimate
librosa-cpp song.wav tempo

# Onset times as JSON for downstream processing
librosa-cpp song.wav --format json onset | python3 -m json.tool

# Extract 13 MFCCs as CSV, no time column
librosa-cpp song.wav --no-time --format csv mfcc --n-mfcc 13 > features.csv

# Compare pitch methods
librosa-cpp song.wav pitch --method pyin > pyin.tsv
librosa-cpp song.wav pitch --method yin  > yin.tsv

# Load at native sample rate, analyze first 10 seconds
librosa-cpp --sr 0 --duration 10 song.wav melspectrogram --format json > mel.json

# Separate harmonic and percussive components
librosa-cpp song.wav hpss -o harmonic.wav --component harmonic
librosa-cpp song.wav hpss -o percussive.wav --component percussive

# Chroma from constant-Q transform
librosa-cpp song.wav chroma --variant cqt

# Custom STFT parameters for all spectral features
librosa-cpp --n-fft 4096 --hop-length 1024 song.wav spectral-centroid
```
